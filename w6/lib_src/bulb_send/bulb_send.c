...
static void send_custom_data(zb_uint8_t param, zb_uint8_t command,
                             zb_uint8_t length, zb_voidp_t ptr);
...
{
    bulb_send_brightness_t *tail = ZB_GET_BUF_TAIL(
        (zb_buf_t *)ZB_BUF_FROM_REF(param), sizeof(bulb_send_brightness_t));
    send_custom_data(param, BULB_BRIGHTNESS_LEVEL, sizeof(tail->brightness),
                     (zb_voidp_t) & (tail->brightness));
}

static void send_simple_command(zb_uint8_t param,
                                zb_uint8_t command) //, zb_uint16_t addr)
{
    ...
    ZB_BUF_INITIAL_ALLOC(
        buf, sizeof(bulb_cmd_t),
        bulb_command_send);       // create buf with size of the sending command
    *bulb_command_send = command; // set command to send

    TRACE_MSG(TRACE_APS2, "### Sending command = %d to address = %d",
              (FMT__D_D, *bulb_command_send, addr_send));
...
static void send_custom_data(zb_uint8_t param, zb_uint8_t command,
                             zb_uint8_t length,
                             zb_voidp_t ptr) // b_uint_t *ptr, zb_uint16_t addr)
{
    zb_buf_t *buf = (zb_buf_t *)ZB_BUF_FROM_REF(param);
    zb_uint16_t addr_send;
    zb_uint16_t *addr_tail;
    ...
    addr_send = *addr_tail; // save address from tail
    ZB_BUF_INITIAL_ALLOC(
        buf, sizeof(bulb_cmd_t) + length,
        bulb_command_send); // create new buf with size of the sending command
    *bulb_command_send = command; // set command to send
    ZB_MEMCPY(bulb_command_send + 1, ptr, length);

    TRACE_MSG(TRACE_APS2,
              "### Sending command = %d to address = %d, payload length = %d",
              (FMT__D_D_D, *bulb_command_send, addr_send,
               sizeof(bulb_cmd_t) + length));
