...
void zc_get_command(zb_uint8_t param) ZB_CALLBACK
{
	zb_uint8_t *ptr;
	bulb_cmd_t *bulb_cmd;
	bulb_data_cmd_param_t *bulb_cmd_param;
	...
	switch(*bulb_cmd) 
	{
		...
		case BULB_BRIGHTNESS_LEVEL:
			bulb_cmd_param = (bulb_data_cmd_param_t *)ZB_BUF_BEGIN(buf);
			TRACE_MSG(TRACE_APS2, "### Received command = %d, brightness = %d", (FMT__D_D,bulb_cmd_param->command,bulb_cmd_param->brightness));	 
		break;
		default:
		break;
	}
